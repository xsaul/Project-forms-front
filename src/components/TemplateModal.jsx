...
useEffect(() => {
  if (isEditing && template) {
    ...
    setQuestionId((template?.questions?.length) || 1);
  }
}, [isEditing, template]);
  ...
  return (
    <Dialog open={isOpen} onClose={onClose} className="fixed inset-0 flex items-center justify-center">
      ...
      <div className="bg-white p-6 rounded-md shadow-lg w-[480px] z-50 overflow-y-auto max-h-[82vh]">
        ...
        <input type="text" className="rounded border px-[5px] py-[3px] border-[#c0d0d9] w-full" value={title} onChange={(e) => setTitle(e.target.value)} required/>
        <label className="block mt-4">Description</label>
        <textarea className="rounded border px-[5px] py-[3px] border-[#c0d0d9] w-full" rows="4" value={description} onChange={(e) => setDescription(e.target.value)} required/>
        <label className="block mt-4">Topic</label>
        <select className="rounded border px-[5px] py-[3px] border-[#c0d0d9] w-full" value={topic} onChange={(e) => setTopic(e.target.value)} required>
        ...
        <label className="flex items-center mt-4">
          <input type="checkbox" className="mr-2" checked={isPublic} onChange={(e) => setIsPublic(e.target.checked)} required/>
          Make the template public?
        </label>
        <label className="block mt-4 mb-1">Upload Image (Optional)</label>
        <input type="file" className="rounded border p-[5px] border-[#c0d0d9] w-full" onChange={(e) => setImage(e.target.files[0])} />
         <div>
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {labels.map((label, index) => (
          <span
            key={index}
            className="bg-gray-300 px-3 py-1 rounded text-sm flex items-center">
            {label}
            <button
